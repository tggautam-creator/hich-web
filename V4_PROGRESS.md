...
| B.6 | Driver request surfacing (FCM badge + request-review rows + earnings) | 🟡 partial (2026-06-05) — **instant suggestion card DONE**: `RideSuggestionPage` now shows a `CompanionContextRow` ("Bringing N companions: names") before accept, fed by `companion_count`/`companion_names` already in the request FCM (A.3); no phones pre-accept (privacy). iOS-only, sim+device build/install OK, lint clean (mine), 6 new `RideRequestPayloadTests` + FareTests 37 green. **Pending:** board request-review surfacing (`BoardReviewRide` + the board-review endpoint needs companion fields — server + iOS). |
