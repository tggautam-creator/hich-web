...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **B.7 pickup drawer done** — the driver now sees companions on the pickup drawer (post-accept), via a new `GET /api/rides/:id/party` server read that also **fixed a pre-existing RLS bug** where the caregiver row never showed for drivers. F1 status: money path complete (A.4); driver visibility live on the instant suggestion card (B.6) + pickup drawer (B.7-pickup). Next F1 slices: **B.7b** (active-ride drawer — identical party-endpoint fix), finish B.6 board request-review, B.8 chat, B.9 summary rows, B.10 board badge; A.6 + B.5 driver waive toggle (A.4 already honors the flag). Web Block C deferred (§2.15). Earlier: **F5 + F4 + A.4 done.**
...
| B.6 | Driver request surfacing (FCM badge + request-review rows + earnings) | 🟡 partial (2026-06-05) — **instant suggestion card DONE + verified by Tarun**: `RideSuggestionPage` shows `CompanionContextRow` ("Bringing N companions: names") before accept (no phones, privacy), AND a "+ $X companion seat fee" line in the earnings breakdown (Tarun feedback — mirror the caregiver fee line; needed `companionFareCents` added to `RideRequestPayload`). Data from request FCM (A.3). iOS-only; sim+device build/install OK; lint clean (mine); 7 `RideRequestPayloadTests` + FareTests 37 green. **Pending:** board request-review surfacing (`BoardReviewRide` + board-review endpoint companion fields). |
| B.7 | Companion context rows on pickup + active-ride drawers | 🟡 partial (2026-06-05) — **pickup drawer DONE**: new server `GET /api/rides/:id/party` (supabaseAdmin, participant-gated, phone post-accept) returns caregiver + companion profiles; `DriverPickupPage+Live` now loads via it (was an RLS-blocked client query — **fixed the broken caregiver row too**) → `DriverJourneyDrawer` shows `CompanionContextRow`. New `RidePartyEndpoint` + 3 `RidePartyResponseTests`. Sim+device build/install OK; lint clean (mine); 47 iOS tests green. **Pending B.7b:** the **active-ride** drawer (`DriverActiveRidePage` + `DriverActiveRideDrawer`) — identical pattern (its caregiver client-fetch at :1142 is still RLS-broken). |
