...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **B.4 (board-request companion toggle) done** — "Traveling with someone?" now on both request surfaces (instant + board). Required a server slice: `/api/schedule/request` now persists + prices companion AND caregiver on the rides row (caregiver-on-board was a pre-existing server drop — fixed per Tarun). Next F1 slices: A.4 end-of-ride recompute (caregiver + companion — convergence zone), A.5 remaining (`/rides/active` companion identity post-accept + server tests), A.6 profile-waive + B.5 driver waive toggle, B.6–B.10 surfacing (lights up the F4 hooks). Web Block C deferred (§2.15). Earlier: **F5 + F4 done + verified.**
