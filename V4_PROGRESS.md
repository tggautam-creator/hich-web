...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **B.8 done — companion identity in chat.** Companion visibility is now complete on EVERY surface: request (B.6) → pickup + active drawers (B.7) → post-ride summary (B.9) → board-card badge (B.10) → **chat thread (B.8)**. The `/party` endpoint underpins all post-accept surfaces. F1 status: money path ✅ (A.4); companion visibility ✅ (B.6/B.7/B.8/B.9/B.10). **Only remaining F1: A.6 + B.5** — the driver waive-companion-fee toggle (A.4 already honors the flag in settlement; A.6 = profile read/write, B.5 = the DriverHome/EditProfile toggle + help box). Optional polish: richer per-companion row (avatar/phone) on drawers/summary. Web Block C deferred (§2.15). Earlier: **F5 + F4 + A.4 done.**
