...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **A.4 (end-of-ride recompute) done** — companion + caregiver seat fees now actually CHARGE at ride end (both QR paths, single + multi-rider), recomputed from real driven distance, with per-fee driver waive. Fixed a pre-existing gap where single-rider caregiver fees were never charged. So F1's money path is now correct end-to-end: attach (B.3/B.4) → price at request (A.3/A.5-board) → **charge at settlement (A.4)**. Next F1 slices: A.5 remaining (`/rides/active` companion identity post-accept + server tests), A.6 profile-waive + B.5 driver waive toggle, B.6–B.10 surfacing (driver SEES companion; lights up the F4 hooks). Web Block C deferred (§2.15). Earlier: **F5 + F4 done + verified.**
