...
**System A — App Gate (force-update + maintenance)**
| # | Slice | Status |
|---|---|---|
| A.1 | Migration: app_gate_config singleton (+seed, store_url) + admin-only RLS | ⬜ |
| A.2 | GET /api/app/gate (semver compare, maintenance, admin bypass) + tests | ⬜ |
| A.3 | Admin gate endpoints + UI (maintenance toggle, versions, copy) + audit ⚠️ admin lane | ⬜ |
| A.4 | iOS gate: launch+foreground+periodic; force-update/maintenance covers + recommended nudge | ⬜ |
| A.5 | Web maintenance gate in AuthGuard (blocking cover + re-check) | ⬜ |
