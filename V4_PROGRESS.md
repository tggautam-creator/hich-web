...
**Current focus:** **F1 (Companion) — backend (A.1+A.2) + iOS companion management (B.1+B.2) done.** Migration 119 applied to dev. Riders can now add/edit/remove companions in Profile (ungated) with a required photo, relationship, and a Contacts-picker phone field. Awaiting Tarun verify (Profile → Companions → Add). Next F1 slices: A.3 fare fn + attach-at-request + FCM, A.4 end-of-ride recompute (caregiver + companion), A.5 board/active-ride identity, A.6 profile-waive; iOS B.3 rider toggle + multi-select up to 2, B.5 driver waive toggle, B.6–B.10 surfacing (lights up the F4 hooks). Web Block C deferred (§2.15). Earlier: **F5 + F4 done + verified.**
