...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **B.10 done + destination-match bug fixed.** Companion visibility is now complete across the whole F1 surface: request (B.6 instant+board) → pickup + active drawers (B.7) → post-ride summary (B.9) → **board-card badge (B.10)**. Also fixed a bug Tarun caught where the board request-review said "Different from your destination" for a rider who chose the driver's destination. F1 status: money path ✅ (A.4); companion visibility ✅ (B.6/B.7/B.9/B.10). Remaining F1: **B.8** chat companion identity, **A.6 + B.5** driver waive-companion toggle (A.4 already honors the flag); optional richer per-companion row (avatar/phone). Web Block C deferred (§2.15). Earlier: **F5 + F4 + A.4 done.**
