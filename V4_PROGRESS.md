...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **B.7 done (both drawers)** — driver sees companions on the pickup AND active-ride drawers (post-accept), via the new `GET /api/rides/:id/party` server read that also **fixed a pre-existing RLS bug** where the caregiver row never showed for drivers. F1 status: money path complete (A.4); driver visibility live on instant suggestion card (B.6) + pickup + active drawers (B.7). Next F1 slices: **B.6 board request-review** (driver reviewing a request on their posted ride — does NOT show companions yet; can reuse the party endpoint), B.8 chat, B.9 summary rows, B.10 board badge; A.6 + B.5 driver waive toggle (A.4 already honors the flag). Web Block C deferred (§2.15). Earlier: **F5 + F4 + A.4 done.**
