...
**Current focus:** **F1 (Companion) — backend (A.1–A.3) + iOS companion management (B.1+B.2) + instant-ride attach (B.3) done.** Migration 119 applied to dev. Riders can add/edit/remove companions in Profile (ungated) and now **attach up to 2 companions on the instant Confirm-ride screen** (toggle → multi-select with avatars → live seat-fee preview; sends `companion_a_id`/`companion_b_id` to `/api/rides/request`, which the server already validates + prices). Sim build installed; awaiting Tarun verify on sim (Confirm ride → "Traveling with companions?"). **B.6 (instant) done** — the driver now SEES who's coming on the instant suggestion card (`CompanionContextRow`, before accept, no phones). F1 money path complete (attach→price→charge, A.4) + first visibility surface live. Next F1 slices: finish B.6 board request-review (server companion fields + `BoardReviewRide`), B.7 (pickup + active-ride drawers — needs `/rides/active` to return companion profiles = A.5-rest), B.8 chat, B.9 summary rows, B.10 board badge; A.6 + B.5 driver waive toggle (A.4 already honors the flag). Web Block C deferred (§2.15). Earlier: **F5 + F4 + A.4 done.**
